...
```
┌────────────────────────────────────────────────┐
│ Web UI(嵌入式静态页面,127.0.0.1,无令牌认证)         │
...
- 守护进程:常驻、开机自启(可选),监听 `127.0.0.1:<端口>`(桌面随机,持久化于 `data_dir/port`,重启/切换用户后端口不变;移动端固定 18571)。**无访问令牌**(token 已废弃,2026-08-20):仅绑回环,验证职责由 PIN 协议承担(plain 用户无验证,v1 接受本地信任模型)。
...
- 功能:会话列表、消息流(分页)、QR 指纹验证、配对模式、设备管理(吊销)、密钥备份/恢复(助记词)、对端阻止/带外验证、传输状态指示(BLE/局域网/互联网/SIG Mesh)、**多用户管理(创建 PIN 用户 / 设置 PIN / 切换用户)与锁定屏(PIN 解锁)**。界面文案走 i18n 键目录(zh-CN/en-US 各 145 键,CI 校验键集合一致,见 webui/src/i18n.rs)。样式:深色/浅色主题(默认跟随系统,可手动切换);图标统一自绘圆滑路径 SVG,**禁止 emoji 作图标**;移动优先响应式(移动端单栏 / 桌面三栏;<1024px 设置视图占满主区域,顶栏 `+`/齿轮入口所有宽度可见)。构建/开发/平台注意详见 [docs/webui.md](docs/webui.md)。
- **移动端(Tauri)**:内嵌守护进程(`zoe-daemon` lib,`default-features=false` 排除 libp2p),固定端口 `127.0.0.1:18571`,WebView 加载该地址(同源免 CORS);UI 启动直连(无登录/令牌步骤);同一份 UI 产物与桌面共用,多用户切换在移动端禁用(见 docs/tauri-mobile.md §0)。
...
- **Q10 多用户形态(已定,2026-08-20)**:单守护进程注册表承担多账号(users.db),每用户独立数据目录 `users/<id>/`;**切换账号 = `POST /users/:id/activate` → 写 `last_used` → daemon 自重启**(桌面 CLI;v1 不提供运行时热切,重启以 `--user` 解析最近使用账号,端口经 `data_dir/port` 持久化保持稳定;移动端内嵌禁用切换)。PIN 用户种子只以加密形式存注册表,明文不落盘。
