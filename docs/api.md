...
守护进程监听 `127.0.0.1:<端口>`,提供静态 Web UI 与 JSON API。**无访问令牌**(token 已废弃,2026-08-20):守护进程仅绑定回环地址,验证职责由 PIN 协议承担(plain 用户无验证,v1 接受本地信任模型)。**端口**:桌面随机为主,持久化到 `data_dir/port`,重启/切换用户后端口不变(Web UI 无需重输地址);移动端固定 `18571`。CORS:仅允许 `Origin: http://127.0.0.1:*` 与 `null`(file:// 场景),其余拒绝。
...
| POST | `/api/v1/users/:id/activate` | 切换活跃用户(仅桌面):更新 `last_used` 并**自重启**,重启后按最近使用激活;移动端 400 |
| POST | `/api/v1/users/:id/set-pin` | 为激活用户设置 PIN:`{pin}`;v1 仅允许激活用户 |
...
| GET | `/api/v1/transports` | 各传输状态:ble/lan/net 是否可用、邻居数(`net_peers`)、打洞状态。**移动端语义(2026-08-20)**:`loopback` 恒 up;`ble` = 宿主应用(回环桥)已连接(`AppState.ble_up`,开机自动 `start_bridge`),`lan`/`net`/`sigmesh` 移动端未挂载特征/驱动,如实 down。`platform` = `mobile`/`desktop`,移动端 UI 据此隐藏桌面专属传输(2026-08-21) |
...
- **切换用户**:**`POST /users/:id/activate`** → 注册表写入 `last_used` → 守护进程以相同数据目录/端口 + `--user <id>` 自重启(仅桌面 CLI;内嵌移动端禁用,返回 400)。重启后该用户成为激活用户;若为 PIN 用户且未携带 `--pin`,进入锁定模式等待解锁。端口经 `data_dir/port` 持久化保持稳定,切换期间 Web UI 轮询恢复。
...
{"type":"user",      "user_id":..., "event":"created|pin_set|switched"}
```
...
- `GET /` → 主界面(锁定模式下渲染锁定屏);无需登录页(token 已废弃)。
...
- **多语言(i18n)**:UI 文案走 `webui/src/i18n.rs` 键驱动词典(zh-CN/en-US 各 145 键,无外部 json);语言优先级 = 用户设置(`settings.ui_language`)> `navigator.language` > 默认 en-US。服务端不参与翻译:API 只返回数据与错误码,文案由客户端映射,切换即时生效、无需重启。键集合完整性由单测 + CI 校验(见 docs/modules.md §4)。
...
- 访问令牌已废弃(2026-08-20):PIN 承担验证职责;plain 用户无验证,v1 接受本地信任模型(绑定回环 + 无远程攻击面)。
