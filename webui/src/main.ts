// zoe-chat Web UI 主应用:登录 → 会话/线程/设置。
// 无框架;视图为模板字符串 + 事件委托;响应式由 CSS 驱动(body[data-view])。

import * as api from './api.js';
import { t, getLang, setLang, Lang } from './i18n.js';
import { getTheme, setTheme, applyTheme, Theme } from './theme.js';
...
const state: {
  ...
  view: View;
  transports: api.TransportStatus | null;
  net: api.NetAddr | null;
} = {
  token: null,
  me: null,
  groups: [],
...
function renderApp(): void {
  const transportDots = (): string => {
    const ts = state.transports;
    const items: Array<[string, keyof api.TransportStatus]> = [
      ['transport.ble', 'ble'],
      ['transport.lan', 'lan'],
      ['transport.net', 'net'],
    ...
    return `<span class="transport-dots" title="${items
      ...
      .map(
        ([, key]) =>
          `<span class="dot ${ts?.[key] === 'up' ? 'up' : 'down'}" title="${esc(t(`transport.${key}`))}"></span>`,
      )
      .join('')}</span>`;
  };
  ...
  document.getElementById('lang-btn')!.addEventListener('click', () => {
    const next: Lang = getLang() === 'zh-CN' ? 'en-US' : 'zh-CN';
    setLang(next);
...
function syncThemeButton(): void {
  ...
  const iconName: IconName = theme === 'dark' ? 'moon' : theme === 'light' ? 'sun' : 'monitor';
...
async function refreshMessages(): Promise<void> {
  ...
  if (!list || !state.currentGroupId) return;
  ...
  const statusText = (s: number): string =>
    s === 0 ? t('chat.pending') : s === 1 ? t('chat.delivered') : s === 3 ? t('chat.failed') : '';
  list.innerHTML = msgs
    .map(
      (m) => `
      <div class="msg ${m.direction === 1 ? 'out' : 'in'}">
        <div class="bubble">${esc(m.text ?? '')}</div>
        <div class="meta">${fmtTime(m.received_at)}${
        m.direction === 1 && statusText(m.status)
...
function renderDetails(): void {
  ...
  document.getElementById('invite-btn')!.addEventListener('click', async () => {
    const input = document.getElementById('invite-addr') as HTMLInputElement;
    const msg = document.getElementById('invite-msg')!;
    const addr = input.value.trim();
    if (!addr || !state.currentGroupId) return;
...
async function renderSettings(): Promise<void> {
    ...
    <div class="panel-section">
      ...
      <div class="row">
        ${(['light', 'dark', 'system'] as Theme[])
          .map(
            (th) =>
              `<button class="${getTheme() === th ? 'primary' : ''}" data-theme-set="${th}">${esc(
                t(`settings.theme.${th}`),
              )}</button>`,
          )
          .join('')}
    ...
    <div class="panel-section">
      ...
      <div class="row">
        ${(['zh-CN', 'en-US'] as Lang[])
          .map(
            (l) =>
              `<button class="${getLang() === l ? 'primary' : ''}" data-lang-set="${l}">${l}</button>`,
          )
          .join('')}
      </div>
    </div>
  ...
  details.querySelectorAll('[data-theme-set]').forEach((b) =>
    b.addEventListener('click', () => {
      setTheme(b.getAttribute('data-theme-set') as Theme);
      void renderSettings();
      syncThemeButton();
    }),
  );
  details.querySelectorAll('[data-lang-set]').forEach((b) =>
    b.addEventListener('click', () => {
      setLang(b.getAttribute('data-lang-set') as Lang);
      void syncSettingsToServer();
      void boot();
    }),
  );
  ...
  document.getElementById('dial-btn')!.addEventListener('click', async () => {
    const input = document.getElementById('dial-addr') as HTMLInputElement;
...
async function refreshTransports(): Promise<void> {
  ...
  if (dots) dots.outerHTML = dots.outerHTML; // 简单重渲染
  renderAppHeader();
}

function renderAppHeader(): void {
  // 通过重新渲染 header 里的 transport dots 保持简单
...
async function syncSettingsToServer(): Promise<void> {
  try {
...
async function boot(): Promise<void> {
  ...
  setLang(getLang()); // 应用已存语言
  applyTheme();
  renderApp();
  setView('thread');

  api.connectEvents((e) => {
    if (e.type === 'message') void refreshMessages();
    if (e.type === 'group') void refreshGroups();
    if (e.type === 'transport') void refreshTransports();
  });
}
