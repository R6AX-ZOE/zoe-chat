...
impl BLEDevice {
    pub async fn new(
        ...
        // zoe-chat patch:默认 MaintainConnection=false 时 Windows 会在 GATT 操作
        // 间隙主动断开 LE 链路(对端看到"连接→立即断开",后续调用报"对象已关闭"/
        // Unreachable)。显式保持连接,修复 Windows 真机联调掉线问题。
        gatt_session
            .SetMaintainConnection(true)
            .map_err(|_| Error::Other("Could not set maintain connection".into()))?;
